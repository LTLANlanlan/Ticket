...
public class FindResultsFrame extends JFrame implements ActionListener{
	...
	private JLabel lab_path = new JLabel("长春--郑州");
	private JButton but_date = new JButton("06-21 周五");
...
JButton[] but_book = new JButton[20];

FindResultsFrame(Plant[] p){
	this.p=p;
	setTopPanel();
	setCenterPanel();
	setLayout(null);
...
public void setComponentText(int i) {
	lab_place[i].setText(p[0].getStart()+"--"+p[0].getEnd());
	//lab_price[i].setText();
...
public void setCenterPanel() {
	...
	for(int i=0;i<2;i++) {  
		  lab_time[i] = new JLabel("2019年6月21日");
          lab_startTime[i]  = new JLabel("10:25出发");
          lab_place[i]  = new JLabel("长春-郑州");
          lab_flightCom[i]  = new JLabel("东方航空G1422");
          lab_price[i]  = new JLabel("¥510");
